...
class ProdutoEconomiaHomeService
{
    private $validator;
    private $mockup;
    private $response;
    ...
    public function __construct($requestTransfer) 
    {
        ...
        $this->mockup = new ProdutoEconomiaHomeMockup();
        $this->response = new ProdutoEconomiaHomeResponse();
        $this->requestTransfer = $requestTransfer;
        $this->responseTransfer = new ProdutoEconomiaHomeResponseTransfer();
    }
            ...
            { 
                ...
                if ($this->validator->validarArray($dadosLista)) {
                    ...
                    { 
                        ...
                        $this->mensagem[] = ProdutoEconomiaHomeException::inexistente(__METHOD__, 'receberSelecionar.inexistente');
                    }
                }

            } else {
                $this->mensagem[] = ProdutoEconomiaHomeException::inexistente(__METHOD__, 'retornarLista.inexistente');
            }

        } catch (Exception $e) {
            $this->mensagem[] = ProdutoEconomiaHomeException::incorreto(__METHOD__, 'service.incorreto=' . $e->getMessage());
        } 
