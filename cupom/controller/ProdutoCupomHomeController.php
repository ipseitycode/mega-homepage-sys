...
class ProdutoCupomHomeController
{
    private $validator;
    private $request;
    ...
    {   
        ...
        $this->request = new ProdutoCupomHomeRequest();
        $this->requestTransfer = $this->request->build($requestTransfer);

        $this->view = new ProdutoCupomHomeView();
    }
    ...
    public function obterSelecionar()
    {
        ...
        try { 
            ...
            $service = new ProdutoCupomHomeService($this->requestTransfer);
            ...
            { 
                ...
                if ($this->validator->validarClasseMetodo($this->view, 'exibirSelecionar')) 
                {
                    ...
                    $this->mensagem[] = ProdutoCupomHomeException::inexistente(__METHOD__, 'exibirSelecionar.inexistente');
                }

            } else {
                $this->mensagem[] = ProdutoCupomHomeException::inexistente(__METHOD__, 'executarSelecionar.inexistente');
            }

        } catch (Exception $e) {
            $this->mensagem[] = ProdutoCupomHomeException::incorreto(__METHOD__, 'controller.incorreto=' . $e->getMessage());
        }
