...
class ProdutoLancamentoHomeService
{
    private $validator;
    private $mockup;
    private $response;
    ...
    public function __construct($requestTransfer) 
    {
        ...
        $this->mockup = new ProdutoLancamentoHomeMockup();
        $this->response = new ProdutoLancamentoHomeResponse();
        $this->requestTransfer = $requestTransfer;
        $this->responseTransfer = new ProdutoLancamentoHomeResponseTransfer();
    }
            ...
            { 
                ...
                if ($this->validator->validarArray($dadosLista)) {
                    ...
                    { 
                        ...
                        $this->mensagem[] = ProdutoLancamentoHomeException::inexistente(__METHOD__, 'receberSelecionar.inexistente');
                    }
                }

            } else {
                $this->mensagem[] = ProdutoLancamentoHomeException::inexistente(__METHOD__, 'retornarLista.inexistente');
            }

        } catch (Exception $e) {
            $this->mensagem[] = ProdutoLancamentoHomeException::incorreto(__METHOD__, 'service.incorreto=' . $e->getMessage());
        } 
